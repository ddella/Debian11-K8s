...
Results (if you disabled IPv6, you won't see the 2nd line):  
...
## Open the Hubble UI
Usually you will never access the Hubble UI directly from a K8s node. You will access it from a management server elsewhere in your network. I made a simple `yaml` file to expose a port on each K8s node with the kind K8s nodePort. You can access the UI with a browser pointing to any K8s worker node's IP address.
...
Point your browser to `http://<K8s node IP>:30012`
